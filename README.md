# YCY-Control
主要针对役次元蓝牙设备开发的控制软件，役次元其余设备在逐渐开发中，未来计划将较火的其他设备也进行接入。

目前支持杯子2.0，包括基础控制和随机控制两种模式，支持命令行和图形界面操作。如需直接使用可下载“杯杯2.0随机控制.exe”文件，无需安装任何依赖。
...
8. **可执行文件校验**：
   - 为了确保程序的完整性和安全性，建议在使用前校验可执行文件的完整性
   - 可以使用如MD5、SHA256等哈希算法校验可执行文件的完整性
   - 该EXE文件的校验值为：`MD5: 28E77FCB29185582420BFB016A20E96F`
   - 该文件大小为：11468KB
